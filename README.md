[![Stata](https://img.shields.io/badge/Stata-17%2B-1A6496?style=flat-square)](stata/)
[![Python](https://img.shields.io/badge/Python-3.11%2B-3776AB?style=flat-square&logo=python&logoColor=white)](src/)
[![License: MIT](https://img.shields.io/badge/License-MIT-green?style=flat-square)](LICENSE)
[![Panel](https://img.shields.io/badge/Panel-EA--20%20%7C%202000--2022-orange?style=flat-square)]()
[![Method](https://img.shields.io/badge/Method-FE%20%2B%20System--GMM-blue?style=flat-square)]()
[![Thesis](https://img.shields.io/badge/MSc%20Thesis-Federico%20II%20Naples-red?style=flat-square)]()
[![AGORA](https://img.shields.io/badge/Extension-AGORA%20ABM-purple?style=flat-square&logo=github)](https://github.com/Leotaby/Agora)
...
These channels produce different predictions for different outcomes. Higher unemployment hurts asset quality directly through the NPL channel, and indirectly reduces capital through earnings erosion. Higher rates compress margins and the Z-score, but the effect on NPLs is ambiguous - rates also improve debt-service capacity for variable-rate borrowers in some circumstances. The causal chain below maps this more explicitly.
...
In the System-GMM specification, a 1 pp rise in the real short rate is associated with log Z falling by 0.10 (p approx. 0.06) and Tier-1 falling by 0.17 pp (p > 0.10, but consistent in direction). Given Tier-1 persistence (rho = 0.929), the implied long-run effect is larger: approximately -2.4 pp per 1 pp real-rate shock, though this should be interpreted cautiously given the standard error.
...
**Supervisory dashboards.** The monitoring set suggested by this paper is four variables: unemployment, GDP growth, real short rate, and credit-to-GDP gap. Their signs are unambiguous and their elasticities are stable enough to plug directly into scenario analysis.
...
The plan is to calibrate AGORA's shock parameters to the actual 2011 crisis episode using EBA transparency exercise data, then compare the simulated contagion path against the realized NPL and Tier-1 trajectories from the panel. That comparison is Paper 1.
...
├── src/macrobank/        Python replication package
...
**Stata** (primary): run scripts in `stata/` in order.

```stata
do stata/Stata_Baseline.do
do stata/Stata_Baseline_FE_GMM.do
do stata/STATA_Baseline_FE_GMM_AR_HSN.do
do stata/STATA_Baseline_FE_GMM_AR_HSN2.do
```
